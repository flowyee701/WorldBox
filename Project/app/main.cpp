...
enum class SpawnMode { CIVILIAN, WARRIOR, BUILD_BARRACKS, SPAWN_TREE, SPAWN_HORSE, SPAWN_METEORITE, SPAWN_ARMAGEDDON };
enum class WarriorRank { WARRIOR, CAPTAIN };
enum class ToolMode { NONE, KILL, WAR, METEORITE, ARMAGEDDON };
                                           ...
                                           bool& loaded)
{
    ...
    if (mode == SpawnMode::SPAWN_METEORITE) {
        loaded = world.meteoriteTexLoaded[0];
        return &world.meteoriteTex[0];
    }

    if (mode == SpawnMode::SPAWN_TREE) {
        loaded = Plant::texturesLoaded;
        return &Plant::texTree;
    }

    if (mode == SpawnMode::SPAWN_HORSE) {
        loaded = Animal::textureLoaded;
        return &Animal::texture;
    }
...
int main() {
    ...
    while (!WindowShouldClose()) {
        ...
        if (appState == AppState::MAP_MENU) {
            ...
            if (appState == AppState::GAME) {
                ...
                if (!toolsOpen) {
                    ...
                    if (IsKeyPressed(KEY_THREE)) {
                        mode = SpawnMode::SPAWN_METEORITE;
                    }

                    if (IsKeyPressed(KEY_FOUR)) {
                        mode = SpawnMode::SPAWN_ARMAGEDDON;
                    }
                    ...
                    if (IsKeyPressed(KEY_SEVEN)) {
                        mode = SpawnMode::SPAWN_TREE;
                    }

                    if (IsKeyPressed(KEY_EIGHT)) {
                        mode = SpawnMode::SPAWN_HORSE;
                    }
                    ...
                    if (IsKeyPressed(KEY_FIVE)) {
                        toolMode = ToolMode::METEORITE;
                        pendingWarSettlementA = -1;
                    }
                    if (IsKeyPressed(KEY_SIX)) {
                        toolMode = ToolMode::ARMAGEDDON;
                        pendingWarSettlementA = -1;
                    }
                }
                ...
                if (IsMouseButtonPressed(MOUSE_BUTTON_LEFT) && !IsMouseButtonDown(MOUSE_BUTTON_RIGHT)) {
                    ...
                    if (mouseWorld.x >= 0 && mouseWorld.x <= world.worldW &&
                        ...
                        if (shiftDown) {
                            ...
                            else if (toolsOpen && toolMode == ToolMode::METEORITE) {
                                world.SpawnMeteorite(mouseWorld);
                            }
                            ...
                            else if (mode == SpawnMode::SPAWN_TREE) {
                                world.SpawnPlant(mouseWorld);
                            }
                            else if (mode == SpawnMode::SPAWN_HORSE) {
                                world.SpawnAnimal(mouseWorld);
                            }
                            else if (mode == SpawnMode::SPAWN_METEORITE) {
                                world.SpawnMeteorite(mouseWorld);
                            }
                        }
                    }
                ...
                // Armageddon mode - spawn meteors continuously
                if (mode == SpawnMode::SPAWN_ARMAGEDDON || (toolsOpen && toolMode == ToolMode::ARMAGEDDON)) {
                    armageddonTimer += dt;
                    float spawnInterval = 0.15f; // Spawn every 0.15 seconds
                    
                    while (armageddonTimer >= spawnInterval) {
                        armageddonTimer -= spawnInterval;
                        Vector2 randomPos = {
                            (float)GetRandomValue(50, world.worldW - 50),
                            (float)GetRandomValue(50, world.worldH - 50)
                        };
                        world.SpawnMeteorite(randomPos);
                    }
                } else {
            ...
            if (!toolsOpen && mode == SpawnMode::SPAWN_TREE) {
                Vector2 mouseWorld = GetScreenToWorld2D(GetMousePosition(), camera);
                DrawCircleV(mouseWorld, 15.0f, Color{100, 200, 100, 180});
            }

            if (!toolsOpen && mode == SpawnMode::SPAWN_HORSE) {
                Vector2 mouseWorld = GetScreenToWorld2D(GetMousePosition(), camera);
                DrawCircleV(mouseWorld, 12.0f, Color{200, 150, 100, 180});
            }

            if (!toolsOpen && mode == SpawnMode::SPAWN_METEORITE) {
                Vector2 mouseWorld = GetScreenToWorld2D(GetMousePosition(), camera);
                DrawCircleV(mouseWorld, 20.0f, Color{255, 100, 50, 180});
            }
            ...
            const char* modeStr = toolsOpen
                    ...
                    : ((mode == SpawnMode::CIVILIAN) ? "Mode: 1 CIVILIAN"
                       ...
                       : (mode == SpawnMode::SPAWN_METEORITE) ? "Mode: 3 METEORITE"
                       : (mode == SpawnMode::SPAWN_ARMAGEDDON) ? "Mode: 4 ARMAGEDDON"
                       : (mode == SpawnMode::SPAWN_TREE) ? "Mode: 7 TREE"
                       : (mode == SpawnMode::SPAWN_HORSE) ? "Mode: 8 HORSE"
            ...
            if (toolsOpen) {
                const char* toolStr =
                        ...
                        (toolMode == ToolMode::METEORITE) ? "Tool: 5 METEORITE" :
                        (toolMode == ToolMode::ARMAGEDDON) ? "Tool: 6 ARMAGEDDON" :
                                                       "Tool: NONE";
                Color toolColor =
                        (toolMode == ToolMode::KILL) ? Color{255, 170, 170, 255} :
                        (toolMode == ToolMode::WAR)  ? Color{255, 210, 120, 255} :
                        (toolMode == ToolMode::METEORITE) ? Color{255, 140, 60, 255} :
                        (toolMode == ToolMode::ARMAGEDDON) ? Color{255, 50, 50, 255} :
                                                       RAYWHITE;
                ...
                else if (toolMode == ToolMode::METEORITE) {
                    DrawText("Click to spawn meteorite", uiX, uiY, 18, Color{255, 200, 120, 255});
                    uiY += spacing;
                }
                else if (toolMode == ToolMode::ARMAGEDDON) {
                    DrawText("ARMAGEDDON IN PROGRESS!", uiX, uiY, 18, Color{255, 50, 50, 255});
                    uiY += spacing;
                    DrawText("All meteors will rain down", uiX, uiY, 16, Color{200, 100, 100, 255});
                    uiY += spacing;
                }
            }
            else if (mode == SpawnMode::BUILD_BARRACKS) {
            ...
            const char* t4="0: Tools | 1-2: Spawn | 3: Meteor | 4: Armagedd | 7: Tree | 8: Horse | 9: Build | K: Rank | A: AUTO | Esc: Pause | F5: Fullscreen";

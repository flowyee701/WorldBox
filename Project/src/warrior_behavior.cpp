...
void WarriorBehavior::Update(World& world, NPC& npc, float dt) {
    ...
    // Find and chop trees if not in combat
    const float TREE_CHOP_DIST = 40.0f;
    for (auto& plant : world.plants) {
        if (plant.health <= 0.0f) continue;
        if (plant.type != PlantType::TREE) continue;
        
        float d = Vector2Distance(npc.pos, plant.position);
        if (d < TREE_CHOP_DIST) {
            // Chop the tree
            plant.health -= dt * 30.0f;
            npc.vel = {0, 0};
            return;
        }
    }
    
    // Move towards tree if nearby
    for (auto& plant : world.plants) {
        if (plant.health <= 0.0f) continue;
        if (plant.type != PlantType::TREE) continue;
        
        float d = Vector2Distance(npc.pos, plant.position);
        if (d < CELL_SIZE * 8.0f && d > TREE_CHOP_DIST) {
            MoveTowards(npc, plant.position, dt, 0.8f);
            return;
        }
    }

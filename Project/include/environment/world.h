...
enum class TileType { 
    DEEP_WATER,    // 0 - Глубокая вода (непроходимая)
    SHALLOW_WATER, // 1 - Мелководье (можно плавать)
    BEACH,         // 2 - Пляж (песок)
    PLAINS,        // 3 - Равнины (трава)
    FOREST,        // 4 - Лес (тёмно-зелёный)
    HILLS,         // 5 - Холмы
    MOUNTAIN,      // 6 - Горы
    SNOW           // 7 - Снежные вершины
};
...
public:
    ...
    static constexpr int METEORITE_FRAMES = 4;
    Texture2D meteoriteTex[METEORITE_FRAMES]{};
    bool meteoriteTexLoaded[METEORITE_FRAMES]{};
    
    // --- Разрушаемая земля ---
    std::vector<bool> terrainDamage; // true = crater, false = normal
    std::vector<Vector2> craterHistory; // positions where craters were created
    
    bool IsCrater(Vector2 worldPos) const;
    bool IsCraterAtTile(int tileX, int tileY) const;
    void UpdateTerrainRegeneration(float dt);
    
    TileType GetTileType(int tileX, int tileY) const;
    TileType GetTileTypeAtPos(Vector2 worldPos) const;
    void GenerateTerrain();
    
    // Массив высот для генерации (0.0 - глубокая вода, 1.0 - горы)
    std::vector<float> terrainHeight;
    float GetTerrainHeight(int tileX, int tileY) const;
    ...
    // --- Метеориты ---
    void LoadMeteoriteSprites();
    void UnloadMeteoriteSprites();
    void UpdateMeteorites(float dt);
    void SpawnMeteorite(Vector2 targetPos);
    void ProcessMeteoriteCollisions(Meteorite& meteorite);
    void CreateCrater(Vector2 worldPos, float radius);
    ...
    void SpawnPlant(Vector2 pos, PlantType type = PlantType::TREE);
